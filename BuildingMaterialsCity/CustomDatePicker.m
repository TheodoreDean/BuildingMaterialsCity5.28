...
//    _yearPicker= [[CustomPickerView alloc] initWithFrame:CGRectMake(_dayImage.size.width + _monthImage.size.width-2*TABLE_RECT_OFFSET, 0, _yearImage.size.width, _yearImage.size.height) background:_yearImage itemVerticalOffset:0.0f andData:years];
    ...
    _mounthPicker = [[CustomPickerView alloc] initWithFrame:CGRectMake(_yearImage.size.width - TABLE_RECT_OFFSET , 0, _monthImage.size.width, _monthImage.size.height)background:_monthImage itemVerticalOffset:0.0f andData:monthNames];
    ...
    NSLog(@"rang.length  =  %d", range.length);
...
{
    NSLog(@"index %d",dial.selectedIndex);//print index
